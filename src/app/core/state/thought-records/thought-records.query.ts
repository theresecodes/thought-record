...
import { EAGER, PROUD } from '../../constants/emotions-wheel.constant';

@Injectable({
  providedIn: 'root'
})
export class ThoughtRecordsQuery {

  getById(id: string) {
    return {
      id,
      date: new Date(),
      title: 'xsdwthis is a long title of a sample thought record. We are just checking if this is alright',
      emotionRecords: [
        {
          emotion: EAGER,
          intensity: 0.80
